    ...
    JPanel createSearchBar() {
        ...
        DocumentListener documentListener = new DocumentListener() {
            ...
            public void changedUpdate(DocumentEvent e) {
                List<Media> filteredList = new ArrayList<>(store.getItemsInStore());
                ...
                if (btnByTitle.isSelected() || btnByCategory.isSelected()) {
                    ...
                    // Lọc theo tiêu đề hoặc danh mục
                    String searchText = textField.getText().toLowerCase();
                    if (btnByTitle.isSelected()) {
                        filteredList = filteredList.stream()
                                .filter(media -> media.getTitle().toLowerCase().contains(searchText))
                                .collect(Collectors.toList());
                    } else if (btnByCategory.isSelected()) {
                        filteredList = filteredList.stream()
                                .filter(media -> media.getCategory().toLowerCase().startsWith(searchText))
                                .collect(Collectors.toList());
                    }
                } else {
                    ...
                    try {
                        float fromCost = tfFrom.getText().isEmpty() ? Float.MIN_VALUE : Float.parseFloat(tfFrom.getText());
                        float toCost = tfTo.getText().isEmpty() ? Float.MAX_VALUE : Float.parseFloat(tfTo.getText());

                        // Loc theo khoang gia tri
                        filteredList = filteredList.stream()
                                .filter(media -> media.getCost() >= fromCost && media.getCost() <= toCost)
                                .collect(Collectors.toList());
                    } catch (NumberFormatException ex) {
                        // Xu ly khi gia tri khong hop le
                        JOptionPane.showMessageDialog(StoreScreen.this,
                                "Please enter valid numeric values for cost range.",
                                "Invalid Input",
